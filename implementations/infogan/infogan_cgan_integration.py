...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.adv_layer = nn.Sequential(nn.Linear(128 * ds_size ** 2 + opt.n_classes, 1))
    ...
    def forward(self, img, labels):
        ...
        # Embed the labels and concatenate with the image feature vector
        labels_embed = self.label_emb(labels).view(labels.size(0), -1)
        d_in = torch.cat((out, labels_embed), -1)
...
for epoch in range(opt.n_epochs):
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        label_input = to_categorical(gen_labels.numpy(), num_columns=opt.n_classes)
        ...
        validity = discriminator(gen_imgs, gen_labels)[0]
        ...
        real_pred = discriminator(real_imgs, labels)[0]
        d_real_loss = adversarial_loss(real_pred, valid)

        # Loss for fake images
        fake_pred = discriminator(gen_imgs.detach(), gen_labels)[0]
        ...
        # Sample labels for information loss
        sampled_labels = Variable(LongTensor(np.random.randint(0, opt.n_classes, batch_size)), requires_grad=False)

        # Sample noise, labels, and code as generator input
        z = Variable(FloatTensor(np.random.normal(0, 1, (batch_size, opt.latent_dim))))
        label_input = to_categorical(sampled_labels.numpy(), num_columns=opt.n_classes)
        code_input = Variable(FloatTensor(np.random.uniform(-1, 1, (batch_size, opt.code_dim))))

        gen_imgs = generator(z, label_input, code_input)
        _, pred_label, pred_code = discriminator(gen_imgs)

        info_loss = lambda_cat * categorical_loss(pred_label, sampled_labels) + lambda_con * continuous_loss(
